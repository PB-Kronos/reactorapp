...
export const PowerGridPanel = (input: Props) => {
  ...
  return <div className="grid gap-6 lg:grid-cols-2">
    ...
    <Card className="border-emerald-500/30 bg-slate-900/70"><CardHeader><CardTitle className="text-emerald-300">Generator / grid</CardTitle></CardHeader><CardContent className="space-y-5"><div className="flex justify-center"><Synchronoscope actualRPM={input.actualRPM} isSynchronized={input.isSynchronized} syncMargin={3} targetRPM={input.targetRPM}/></div><div className="flex justify-center"><Button variant="outline" onClick={() => setCloseupOpen(true)}>OPEN CLOSE-UP SYNCHROSCOPE</Button></div><Dialog open={closeupOpen} onOpenChange={setCloseupOpen}><DialogContent className="max-w-2xl border-cyan-500/40 bg-slate-950 text-slate-100"><DialogHeader><DialogTitle className="text-cyan-300">Generator synchronizing instrument</DialogTitle><DialogDescription className="text-slate-400">Close the grid breaker only as the needle enters the green window and the phase difference approaches 0°.</DialogDescription></DialogHeader><div className="flex justify-center"><Synchronoscope closeup actualRPM={input.actualRPM} isSynchronized={input.isSynchronized} syncMargin={3} targetRPM={input.targetRPM}/></div></DialogContent></Dialog><div className="grid grid-cols-3 gap-3"><div className="rounded bg-slate-950 p-3">RPM<br/><strong>{input.actualRPM.toFixed(0)}</strong></div><div className="rounded bg-slate-950 p-3">TURBINE FLOW<br/><strong>{(input.turbineSteamFlow ?? 0).toFixed(0)} kg/s</strong></div><div className="rounded bg-slate-950 p-3">OUTPUT<br/><strong>{input.turbineOutputMW.toFixed(1)} MW</strong></div></div><p className="text-xs text-slate-400">Shaft speed follows turbine steam flow before synchronizing; the grid holds 3,000 RPM after the breaker closes.</p><div className="grid grid-cols-2 gap-4"><MaintainedSwitch label="EXCITER BREAKER" on={p.exciter} onChange={p.exciterChange}/><div className="flex flex-col items-center"><SpringButton disabled={!p.exciter || !input.isSynchronized} onClick={p.grid} label={p.gridClosed ? "OPEN GRID BREAKER" : "CLOSE GRID BREAKER"}/><span className={p.gridClosed ? "mt-2 text-xs text-emerald-400" : "mt-2 text-xs text-amber-400"}>{p.gridClosed ? "GRID CONNECTED" : "GRID OPEN"}</span></div></div></CardContent></Card>
  </div>;
};
