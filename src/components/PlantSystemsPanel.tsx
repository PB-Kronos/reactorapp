...
interface Props {
  panel: ProcessPanel;
  condenserVacuum: number; condenserPumpOn: boolean; condenserPumpB: boolean; condenserValve: number; condenserValveDirection?: number; condenserCirculationPumpOn?: boolean; onCondenserCirculationPumpChange?: (v: boolean) => void; busBAvailable: boolean; sjaeOn: boolean; carAOn?: boolean; carBOn?: boolean;
  deaeratorLevel: number; feedwaterDemand: number; feedwaterPumpBFlow?: number; pump1Online?: boolean; pump2Online?: boolean; mccLevel: number; mccPumpOn: boolean; mccAutoOn?: boolean;
...
export const PlantSystemsPanel = (p: Props) => {
  ...
  if (p.panel === "condenser") return <div className="grid gap-6 lg:grid-cols-2">
    <Card className="border-sky-500/30 bg-slate-900/60"><CardHeader><CardTitle className="flex gap-2 text-sky-300"><Fan/>Condenser controls</CardTitle></CardHeader><CardContent className="space-y-5">
      ...
      <MaintainedSwitch label="CONDENSER CIRCULATION PUMP" on={p.condenserCirculationPumpOn ?? false} onChange={value => p.onCondenserCirculationPumpChange?.(value)}/>
    ...
    <Card className="border-slate-700 bg-slate-900/60"><CardHeader><CardTitle>Condenser status</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex justify-between"><span>Pump B supply</span><Badge className={p.busBAvailable ? "bg-emerald-700" : "bg-red-700"}>{p.busBAvailable ? "BUS B AVAILABLE" : "NO BUS B"}</Badge></div><p className="rounded border border-slate-700 bg-slate-950/50 p-4 text-sm text-slate-300">Normal condenser vacuum is 40–70 mbar. The spring lever holds the valve in motion only while pressed.</p></CardContent></Card>
  </div>;
  ...
  if (p.panel === "mcc") return <div className="grid gap-6 lg:grid-cols-2">
    ...
    <Card className="border-slate-700 bg-slate-900/60"><CardHeader><CardTitle>Mass-flow balance</CardTitle></CardHeader><CardContent className="space-y-4">
      <FlowMeter label="TOTAL STEAM FLOW" value={p.steamFlow}/><FlowMeter label="HOTWELL OUTFLOW" value={hotwellOutflow}/><FlowMeter label="DEAERATOR OUTFLOW" value={daOutflow}/>
      <div className="rounded border border-slate-700 p-3"><div className="mb-2 flex justify-between"><span>CONDENSATE PUMP A</span><strong>{p.condensateFlow.toFixed(1)}% / {(p.condensateFlow * 10).toFixed(0)} kg/s</strong></div><Slider value={[p.condensateFlow]} max={100} step={0.1} disabled={!p.condenserPumpOn} onValueChange={v => p.onCondensateFlowChange(v[0])}/><div className="mt-3 flex justify-between"><span>CONDENSATE PUMP B</span><strong>{(p.condensatePumpBFlow ?? 0).toFixed(1)}% / {((p.condensatePumpBFlow ?? 0) * 10).toFixed(0)} kg/s</strong></div><Slider value={[p.condensatePumpBFlow ?? 0]} max={100} step={0.1} disabled={!p.busBAvailable || !p.condenserPumpB} onValueChange={v => p.onCondensatePumpBFlowChange?.(v[0])}/></div>
      ...
      <p className="text-xs text-slate-400">Each pump supplies up to 1,000 kg/s at 100.0%. Guide equations: ΔH = TF − HF, ΔD = HF − DF, ΔR = DF − TF.</p>
    </CardContent></Card>
  </div>;
